...
| Modal target changes during the same resolution | `ModalIntraResolutionPartialTargetTest` | Implemented, including blink/new-object identity; independently reviewed and focused-tested |
...
| Simultaneous combat excess to battles / planeswalkers | `BattleCardTypeScenarioTest`, `TriggerDetectorBatchTriggerTest` | Implemented for aggregate counter removal and generic `Any`/`AnyPermanent` recipient-side excess observers; focused-tested. Source-filtered, batch-worded, controller-relative, and predicate-filtered excess triggers remain unsupported for a multi-source aggregate pending recipient LKI, and source share must never be invented. |
