 ...
 * optional [TransformEffect] flips it. That matters in this set: putting Bruce Banner in and
 * transforming him is not the same as putting The Incredible Hulk directly onto the battlefield.
 *
 * **Known limitation:** the SDK has no "is a double-faced card" predicate, so the "you may
 * transform it" prompt is offered for whatever was put onto the battlefield rather than only for
 * a double-faced card. Declining is always correct for a single-faced permanent and transforming
 * one is a no-op, so the *outcome* is faithful — but the prompt is cosmetic noise there. A
 * `StatePredicate.IsDoubleFaced` would let this be gated properly.
 */
